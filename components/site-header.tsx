...
const navLinks: NavigationLink[] = [
  { label: 'Learning ideas', href: '#early-years-explorer' },
  { label: 'Adventures', href: '#community-adventures' },
...
export function SiteHeader(): ReactElement {
  ...
  const navigateToSection = (event: ReactMouseEvent<HTMLAnchorElement>, href: string): void => {
  ...
  return (
    <header ref={headerRef} className="mx-auto w-full">
      <div className="hidden w-[95%] max-w-[1400px] items-center gap-4 px-5 py-5 lg:mx-auto lg:flex lg:px-10 lg:py-7">
        ...
        <nav className="mx-auto flex items-center gap-7">{navLinks.map((link) => <a key={link.label} href={link.href} onClick={(event) => navigateToSection(event, link.href)} className="relative py-1 text-[0.95rem] font-medium text-foreground/80 transition-colors after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-full after:origin-left after:scale-x-0 after:bg-[#7A997D] after:transition-transform after:duration-300 hover:text-foreground hover:after:scale-x-100">{link.label}</a>)}</nav>
